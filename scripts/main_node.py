...
config_dict = rospy.get_param('/ema/rowing')
...
if __name__ == '__main__':
    try:
        ...
        mserver = multiprocessing.Process(target=server, args=('', ports['mainport']))
        # mserver = threading.Thread(target=server, args=(('', 50001),))
        mserver.start()
        sserver = multiprocessing.Process(target=socket_server, args=('', ports['auxport1'], x, 1))
        sserver.start()
        sserver2 = multiprocessing.Process(target=socket_server, args=('', ports['auxport2'], x, 2))
        sserver2.start()
        # server(('', 50000))
        if real_time_plot:
